...
typedef struct {
	int next_node, merge_to, startORend;
} node;
// navigate shortcut with merged destination
typedef struct {
	int start, dest, disp , proceeded;
} navigator;
...
int main()
{
	...
	int node_counts = 0;
	fscanf( pInputFile, "%d", &node_counts);      // input the numeber of nodes
	...
	navigator nav[100] = {};
	for(int i = 0; i < 100; i++){
		nav[i].start = nav[i].dest = nav[i].disp = nav[i].proceeded = 0;
	}
...
int find_longest(int *curLongestRec, navigator nav[], int shortcutCounter) {
	...
	for (int i = 0; i < shortcutCounter; i++) {
		// after being proceeded the nav's proceeded element will be -1 or 1
